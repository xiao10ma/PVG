...
def visualize_depth(depth, near=2, far=80, linear=False, scale_factor=None):
...
def save_ply_old(points, filename, rgbs=None, loading_bar=True, max_points=1e10):
    if type(points) is np.ndarray:
        points = torch.from_numpy(points)
    if type(rgbs) is np.ndarray:
    ...
    with open(filename, 'w') as f:
        f.write(ply_header)
        if loading_bar:
            bar = tqdm(range(min(num_points, int(max_points))))
        else:
            bar = range(min(num_points, int(max_points)))
        for i in bar:
            point = points[i]
            rgb = rgbs[i]
