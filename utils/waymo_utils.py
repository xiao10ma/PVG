...
def build_pointcloud(args, datadir, object_tracklets_vehicle, object_info, selected_frames, ego_frame_poses):
    start_frame, end_frame = selected_frames[0], selected_frames[1]
    ...
    for i, frame in tqdm(enumerate(range(start_frame, end_frame + 1))):
        ...
        points_xyz_world = points_xyz_vehicle @ ego_pose.T # (xyz.T @ ego_pose.T) = (ego_pose @ xyz).T
        ...
        for tracklet in object_tracklets_vehicle[i]:
            ...
            if track_id >= 0:
                ...
                vehicle2local = np.linalg.inv(obj_pose_vehicle) # ego -> object

                points_xyz_obj = points_xyz_vehicle @ vehicle2local.T
                points_xyz_obj = points_xyz_obj[..., :3] # object coordinate
    ...
    for k, v in points_xyz_dict.items():
        if len(v) == 0:
            ...
            if k == 'bkgd':
                ...
                points_xyz_dict[k] = points_lidar_xyz
    ...
    for k in points_xyz_dict.keys():
        ...
        try:
            storePly(ply_path, points_xyz, np.ones_like(points_xyz))
